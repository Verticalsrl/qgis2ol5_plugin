...
class FolderExporter(Exporter):
    ...
    def readFromProject(self):
        folder, ok = QgsProject.instance().readEntry("qgis2ol5",
                                                     "Exportfolder")
        if ok and folder:
            self.folder = folder
...
class FtpExporter(Exporter):
    ...
    def readFromProject(self):
        host, ok = QgsProject.instance().readEntry("qgis2ol5",
                                                   "FtpHost")
        if ok and host:
            self.host = host
        user, ok = QgsProject.instance().readEntry("qgis2ol5",
                                                   "FtpUser")
        if ok and user:
            self.username = user
        folder, ok = QgsProject.instance().readEntry("qgis2ol5",
        ...
        port, ok = QgsProject.instance().readNumEntry("qgis2ol5",
                                                      "FtpPort")
        if ok and port:
            self.port = port
...
class ExporterRegistry(QObject):
    ...
    def createFromProject(self):
        ...
        :return: new exporter
        """
        type, ok = QgsProject.instance().readEntry("qgis2ol5",
                                                   "Exporter")
        exporter = None
        if ok and type:
            try:
